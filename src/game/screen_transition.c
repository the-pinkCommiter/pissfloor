...
Vtx *vertex_transition_color(struct WarpTransitionData *transData, u8 alpha) {
    ...
    if (verts != NULL) {
        ...
        make_vertex(verts, 2, GFX_DIMENSIONS_FROM_RIGHT_EDGE(0), SCREEN_HEIGHT, -1, 0, 0, r, g, b,
                    alpha);
        make_vertex(verts, 3, GFX_DIMENSIONS_FROM_LEFT_EDGE(0), SCREEN_HEIGHT, -1, 0, 0, r, g, b,
                    alpha);
    } else {
    }
    return verts;
...
s32 render_screen_transition(s8 fadeTimer, s8 transType, u8 transTime,
                             struct WarpTransitionData *transData) {
    switch (transType) {
